...
const float Ball::mRadius = 5.0f;

Ball::Ball() : Ball(Vec2D::Zero, Ball::mRadius)
{
}

Ball::Ball(const Vec2D& pos, float radius) :
	mBBox(pos - Vec2D(radius, radius), radius * 2.f, radius * 2.f)
{
}
...
void Ball::MakeFlushWithEdge(const BoundaryEdge& edge, Vec2D& edgePoint, bool limitToEdge)
{
	...
	edgePoint = edge.edge.ClosestPoint(mBBox.GetCenterPoint(), limitToEdge);
}
