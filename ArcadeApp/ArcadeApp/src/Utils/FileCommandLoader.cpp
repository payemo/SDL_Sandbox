...
bool FileCommandLoader::LoadFile(const std::string& filePath)
{
	...
	while (!inFile.eof())
	{
		...
		{
			size_t delimPos = line.find_first_of(" ", commandPos);
			...
			std::string commandStr = line.substr(commandPos + 1, delimPos);
			delimPos += 1;
			...
			for (size_t commandIndex = 0; commandIndex < mCommands.size(); ++commandIndex)
			{
				...
				{
					if (mCommands[commandIndex].type == COMMAND_ONE_LINE)
					{
					...
					{
						std::string numLines = line.substr(delimPos + 1);
...
{
	std::string intStr = params.line.substr(params.delimPos + 1);
	return std::stoi(intStr);
}

std::string FileCommandLoader::ReadString(const ParseFuncParams& params)
{
	return params.line.substr(params.delimPos + 1);
}

char FileCommandLoader::ReadChar(const ParseFuncParams& params)
{
	return params.line.substr(params.delimPos + 1)[0];
}