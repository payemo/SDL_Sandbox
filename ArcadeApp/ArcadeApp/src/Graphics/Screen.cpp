...
SDL_Window* Screen::Init(uint32_t w, uint32_t h, uint32_t mag, bool accelerate)
{
	...
	if (mWindow)
	{
		...
		//auto windowPixFormat = SDL_GetWindowPixelFormat(mWindow);
...
void Screen::Draw(const BMPImage& image, const Sprite& sprite, const Vec2D& pos, const Color& overlayColor)
{
	...
	FillPoly(points, [&](uint32_t px, uint32_t py)
	{
		...
		Color imageColor = pixels[Get2DIndex(image.GetWidth(), ty + sprite.yPos, tx + sprite.xPos)];
...
void Screen::Draw(const BitmapFont& font, const std::string& textLine, const Vec2D& pos, const Color& overlayColor)
{
	...
	for (auto c : textLine)
	{
		...
		Draw(img, sprite, Vec2D(xPos, pos.GetY()), overlayColor);
